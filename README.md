...
assigns value to all lanes for which 'predicate' is satisfied. Because this is such a succinct way of coding the operation, I have adopted it in zimt and provide it as a common feature for all backends. We can now use the functor to process both scalar and simdized data:
...
What do I mean by 'building block'? zimt offers 'chaining' of functors. This combines two zimt::unary_functors so that the first one's output is taken as the second one's imput. The combined functor is a new unary functor in it's own right. using 'f' from above:
