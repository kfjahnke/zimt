...
The code for tiled storage and the get_t and put_t objects is all in tiles.h, and there are also already some examples. The design is 'solidifying' - there are separate classes for the tiles and the tile store, a common base class limiting the amount of tiles which are currently open, and finally the get_t and put_t objects. The examples merely aim at 'excercising' the code to make sure it compiles okay and runs smoothly.

I think I have found a satisfactory and simple solution to make the code process the set of tiles with normal zimt::process logic (per-segment access) and several worker threads with a two-stage access mediation scheme: processing threads can detect when the segments they process change to access a new row of tiles. While the row remains the same, they hold pointers to tiles they have visited, and they can re-access them quickly without any need for inter-thread communication. When the row changes, this 'working set' of tiles is examined, and if the tiles are found to have no other current users, they are sent to the next stage: the 'limbo'. There, they persist until the limbo overflows, which is when they are finally written to mass storage. This limbo stage reduces store-then-reload situations, down to avoiding them altogether if the size of the limbo is large enough. These situations arise when working threads are halted by the OS's scheduler before they can work over an entire segment: then, they have tiles 'in the pipeline' which they haven't yet visited but need to visit to complete the 'joblet' they picked. Other threads will run on, to the point where these tiles have no current users, which is when they are sent to the 'limbo'. If they persist in limbo until the halted thread is reactivated by the scheduler, the thread can carry on without much delay - as opposed to having to reload a tile from disk which was already flushed out, which would take a long time. I'll examine the performance of this scheme some more, maybe it's not yet the final solution I'll settle on but it's looking quite good. So this is work in progress, expect the code to 'fluctuate' some more:

Of course there are other schemes to avoid the problem, which is really intrinsic to my design. If, for example, the worker threads are made to process whole tiles rather than 'segments' the problem can be avoided altogether. If no threads are scheduled out by the OS, the problem is also mitigated, and this may be a good option if the process is mainly I/O-bound, which is not unlikely - a reasonable compromise being a job count one less than the system has cores (set 'njobs' in the bill). Another option would be workload subdivision and running the partial workloads with only a single thread each but in parallel.
